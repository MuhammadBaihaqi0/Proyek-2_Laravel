<x-app-layout>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4">
                <div class="dashboard-content">
                    ...
                    <div
                        ...
                        <div class="p-8 md:flex items-center justify-between relative z-10">
                            <div class="flex items-center space-x-6">
                                <div class="relative">
                                    <div
                                        class="absolute -inset-1 bg-gradient-to-r from-yellow-400 to-pink-500 rounded-full blur opacity-50">
                                    </div>
                                ...
                                <div>
                                    <h3 class="text-3xl font-bold tracking-tight">{{ $greeting }},
                                        {{ $user->username }}! {{ $icon }}</h3>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in delay-100">
                        <div
                            class="bg-white p-4 rounded-xl shadow-sm border-b-4 border-indigo-500 flex items-center justify-between hover:shadow-md transition">
                            <div>
                        ...
                        <div
                            class="bg-white p-4 rounded-xl shadow-sm border-b-4 border-emerald-500 flex items-center justify-between hover:shadow-md transition">
                            <div>
                        ...
                        <div
                            class="bg-white p-4 rounded-xl shadow-sm border-b-4 border-orange-400 flex items-center justify-between hover:shadow-md transition">
                            <div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in delay-200">
                        <div
                            ...
                            <div class="p-6 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                                ...
                                <span class="text-xs font-semibold bg-gray-200 text-gray-600 px-2 py-1 rounded">Pending:
                                    {{ count($tugas) }}</span>
                            </div>

                            <div class="p-6 flex-grow flex flex-col">
                                <form action="{{ route('tugas.store') }}" method="POST"
                                    class="mb-6 bg-indigo-50/50 p-4 rounded-xl border border-indigo-100">
                                    @csrf
                                    <label class="block text-xs font-bold text-indigo-600 uppercase mb-2">Tambah
                                        Baru</label>
                                        ...
                                        <div
                                            ...
                                            <div class="flex items-center gap-2">
                                                <form action="{{ route('tugas.selesai', $t->id) }}" method="POST">
                                                    @csrf @method('PATCH')
                                                    <button type="submit"
                                                        class="text-xs bg-green-100 text-green-700 px-3 py-1.5 rounded-md hover:bg-green-200 transition font-bold shadow-sm">✅
                                                        Selesai</button>
                                                </form>
                                    ...
                                    @empty
                                        <div class="text-center py-10 opacity-60">
                                            <img src="https://cdn-icons-png.flaticon.com/512/7486/7486744.png"
                                                class="w-16 h-16 mx-auto mb-3 opacity-50" alt="Empty">
                        ...
                        <div
                            ...
                            <div class="p-6 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                                ...
                                <span
                                    class="text-xs font-semibold bg-emerald-100 text-emerald-700 px-2 py-1 rounded">Total:
                                    {{ count($acara) }}</span>
                            </div>

                            <div class="p-6 flex-grow flex flex-col">
                                <form action="{{ route('acara.store') }}" method="POST"
                                    class="mb-6 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                                    @csrf
                                    <label class="block text-xs font-bold text-emerald-600 uppercase mb-2">Jadwalkan
                                        Acara</label>
                                    <div class="flex flex-col gap-3">
                    ...
                    <div class="pt-8 border-t-2 border-dashed border-gray-300">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="bg-gray-100 p-6 rounded-2xl border border-gray-200 shadow-inner">
                                <h4 class="font-bold text-gray-500 uppercase text-xs mb-4 tracking-wider">Tugas Selesai
                                </h4>
                                <ul class="space-y-2 max-h-[200px] overflow-y-auto no-scrollbar">
                                    @forelse($riwayat_tugas as $rt)
                                        <li
                                            class="flex justify-between items-center text-sm bg-white p-3 rounded-lg shadow-sm opacity-70 hover:opacity-100 transition">
                                            <span
                                                class="line-through text-gray-500 flex-1 truncate mr-2">{{ $rt->nama_tugas }}</span>
                                            <span
                                                class="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-bold flex-shrink-0">DONE</span>
                                        </li>
                                    @empty
                                        <li class="text-sm text-gray-400 italic text-center py-4">Belum ada tugas
                                            selesai.</li>
                                    @endforelse
                                </ul>
                            </div>
                            ...
                            <div class="bg-gray-100 p-6 rounded-2xl border border-gray-200 shadow-inner">
                                <h4 class="font-bold text-gray-500 uppercase text-xs mb-4 tracking-wider">Acara
                                    Terlewat</h4>
                                <ul class="space-y-2 max-h-[200px] overflow-y-auto no-scrollbar">
                                    @forelse($riwayat_acara as $ra)
                                        <li
                                            class="flex justify-between items-center text-sm bg-white p-3 rounded-lg shadow-sm opacity-70 hover:opacity-100 transition">
                                            <span
                                                class="text-gray-600 font-medium flex-1 truncate mr-2">{{ $ra->nama_acara }}</span>
                                            <span
                                                class="text-xs text-gray-400 flex-shrink-0">{{ \Carbon\Carbon::parse($ra->tanggal)->format('d M Y') }}</span>
                                        </li>
                                    @empty
                                        <li class="text-sm text-gray-400 italic text-center py-4">Belum ada acara
                                            lewat.</li>
                                    @endforelse
                                </ul>
                            </div>
        ...
        <div class="pt-8 border-t-2 border-dashed border-gray-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-emerald-50 p-6 rounded-2xl border border-emerald-200 shadow-md">
                    <h4
                        class="font-bold text-emerald-700 uppercase text-xs mb-3 tracking-wider flex items-center gap-1">
                ...
                <div class="bg-pink-50 p-6 rounded-2xl border border-pink-200 shadow-md">
                    ...
                    <p class="text-sm text-pink-600 mt-2">Tugas aktif. Persentase selesai: <span
                            class="font-bold">{{ $ratio }}%</span></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <footer class="bg-white border-t border-gray-200 py-6 mt-auto">
        <div class="text-center text-gray-500 text-xs">
            &copy; {{ date('Y') }} Project Laravel by <span
                class="font-bold text-indigo-600">{{ $user->username }}</span>.
        </div>
    </footer>
